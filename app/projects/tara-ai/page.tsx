...
/**
 * Tara AI - Custom SVG Brand Mark
 * An interlocking four-fold stellar knot inspired by the sacred geometry of the guiding star.
 * Combines continuous ribbon topology with a central diamond aperture.
 */
function TaraLogo({ 
  className = "w-6 h-6", 
  size 
  ...
  return (
    ...
    >
      ...
      <path
        ...
        className="fill-[#0F172A] dark:fill-white transition-colors"
      />
      {/* East Petal Loop */}
      <path
        d="M33 18C33 20.8 30.6 23 26.8 23C21.5 23 18 18 18 18C18 18 21.5 13 26.8 13C30.6 13 33 15.2 33 18Z"
        className="fill-[#1E3A8A] dark:fill-slate-300 transition-colors"
      />
      {/* South Petal Loop */}
      <path
      ...
      <path
        ...
        className="fill-[#1E3A8A] dark:fill-slate-300 transition-colors"
      />
      {/* Central Star Core Diamond Aperture */}
      <path
        d="M18 14.5L21.5 18L18 21.5L14.5 18L18 14.5Z"
        className="fill-white dark:fill-[#080D1A] transition-colors"
      />
      {/* Focal Point Center */}
      <circle 
        cx="18" 
  ...
  return (
    <div className={`h-screen w-screen flex overflow-hidden font-sans select-none transition-colors duration-200 ${
      ...
      <aside className={`w-[64px] shrink-0 border-r flex flex-col items-center justify-between py-4 z-20 transition-colors ${
        ...
        {/* Top Tara Logo + Navigation Stack */}
        <div className="flex flex-col items-center gap-5">
          
          {/* Custom Tara AI SVG Logo Button */}
          <button 
            onClick={handleStartNewChat}
            className="w-10 h-10 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 flex items-center justify-center transition-transform hover:scale-105 active:scale-95"
            title="Tara AI Home"
          >
          ...
          <div className="flex flex-col items-center gap-2.5">
            ...
            {/* 1. Chat Icon (Toggles expanded sidebar) */}
            <button 
              onClick={() => {
                setActiveTab("chat");
        ...
        {/* Bottom Rail Controls: Theme toggle & Avatar */}
        <div className="flex flex-col items-center gap-3.5">
          <button 
            onClick={() => setIsDarkMode(!isDarkMode)}
            className="text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 p-1.5 transition-colors rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800"
            title="Toggle Theme"
          >
      ...
      >
        <div className="w-[260px] h-full flex flex-col justify-between py-5 px-3.5 select-none">
          ...
          <div className="flex flex-col gap-4">
            ...
            {/* Chat History List */}
            <div className="flex flex-col gap-4 overflow-y-auto max-h-[calc(100vh-200px)] pr-1 scrollbar-none text-xs">
      ...
      {/* 3. MAIN WORKSPACE (Pure full height, clean header with Tara logo) */}
      <main className={`flex-1 flex flex-col justify-between overflow-y-auto relative transition-colors ${
        ...
        {/* Header Bar */}
        <header className={`h-14 px-8 flex items-center justify-between shrink-0 sticky top-0 z-10 border-b ${
        ...
        <div className="flex-1 flex flex-col px-6 sm:px-12 max-w-4xl mx-auto w-full justify-between pb-8 pt-4">
          ...
          {chatMessages.length === 0 ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center my-auto py-8">
              ...
              {/* Central Iconic Tara Logo */}
              <div className="mb-6 flex items-center justify-center">
                <div className="w-16 h-16 rounded-2xl bg-slate-50 dark:bg-[#0B132B] border border-slate-200 dark:border-slate-800 flex items-center justify-center">
                  <TaraLogo className="w-9 h-9" />
                </div>
              </div>
            ...
            <div className="flex-1 flex flex-col space-y-6 py-6 overflow-y-auto">
                ...
                >
                  <div className={`w-8 h-8 rounded-lg shrink-0 flex items-center justify-center text-xs font-semibold ${
                    msg.role === "user" 
                      ...
                      : "bg-slate-50 text-[#0F172A] dark:bg-slate-800 dark:text-white border border-slate-200 dark:border-slate-700"
                  }`}>
              ...
              {isGenerating && (
                <div className="flex items-center gap-3 mr-auto text-xs text-slate-400">
                  <div className="w-7 h-7 rounded-lg bg-slate-50 dark:bg-slate-800 text-[#0F172A] dark:text-white flex items-center justify-center animate-pulse border border-slate-200 dark:border-slate-700">
                    <TaraLogo className="w-4 h-4" />
                  </div>
